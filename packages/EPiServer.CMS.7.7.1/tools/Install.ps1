...
if ($epiConnection -eq $null)
{
	...
	WRITE-HOST "Running Aspnet Workflow into $($uniqueDB)"
	&$EpidpeployEXEPath  -a sql -s $sitePath  -p (join-path (GetAspNetWorkflowSQLPath)  "SqlPersistenceService_Schema.sql") -t false -v false -c "EPiServerDB"
	&$EpidpeployEXEPath  -a sql -s $sitePath  -p (join-path (GetAspNetWorkflowSQLPath)  "SqlPersistenceService_Logic.sql")  -t false -v false -c "EPiServerDB"
